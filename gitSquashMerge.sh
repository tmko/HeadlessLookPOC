...
if ! echo "$output" | grep -q 'nothing to commit, working tree clean'; then
  echo "local changes are found.  Please commit and push"
  exit
fi
...
if [ $? -ne 0 ]; then
	...
	echo "Rolled back all action! Aborted!"
	exit
fi
...
if [ $? -ne 0 ]; then
	echo "Something wrong with git push" $tempBranch
...
echo "Done git squash commit, Please create a pull request"